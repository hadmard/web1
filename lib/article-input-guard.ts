...
type DirtyTextIssue = "escaped_unicode" | "repeated_question_marks";
...
const REPEATED_QUESTION_MARK_RE = /[?？]{3,}/;
...
function detectDirtyTextIssue(value: string): DirtyTextIssue | null {
  ...
  if (REPEATED_QUESTION_MARK_RE.test(plain)) return "repeated_question_marks";
  return null;
}
...
export function buildDirtyTextErrorMessage(fields: DirtyTextField[]): string | null {
  ...
  return `检测到疑似乱码或脏文本，请检查以下字段：${summary}。请不要提交包含连续问号或 \\uXXXX 编码字面量的内容。`;
}
